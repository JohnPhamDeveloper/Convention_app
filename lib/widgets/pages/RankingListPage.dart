...
class _RankingListPageState extends State<RankingListPage>
  ...
  void initState() {
    super.initState();
    friendlinessCards.add(SizedBox(width: 20.0));
    try {
      // GEt data from database sorted by friendliness
      Firestore.instance
          .collection("users")
          .orderBy(FirestoreManager.keyFriendliness, descending: true)
          .getDocuments()
          .then((snapshot) {
        snapshot.documents.forEach((data) {
          String url = data[FirestoreManager.keyPhotos][0];
          RankCard card = RankCard(
            image: NetworkImage(url),
            name: data[FirestoreManager.keyDisplayName],
            icon: Icons.sentiment_very_satisfied,
            value: data[FirestoreManager.keyFriendliness],
            dotIsOn: true,
            key: UniqueKey(),
          );
          setState(() {
            friendlinessCards.add(card);
          });
        });
      });
    } catch (e) {
